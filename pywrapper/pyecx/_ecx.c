...
static PyObject *_ecx_kh(PyObject *self, PyObject *args){
    ...
    if(PyFloat_Check(T_obj) == 1){
        value = ecx_capi_kh(PyFloat_AsDouble(T_obj), gas, solvent, strlen(gas), strlen(solvent));
        return Py_BuildValue("d", value);
    }else if(PyLong_Check(T_obj)==1){
        value = ecx_capi_kh(PyLong_AsDouble(T_obj), gas, solvent, strlen(gas), strlen(solvent));
        return Py_BuildValue("d", value);
        ...
        if(strcmp(buffer->format, "d")!=0){
            ...
            for(i=0; i<buffer->shape[0]; i++){
                ...
                value = ecx_capi_kh(T, gas, solvent, strlen(gas), strlen(solvent));
...
static PyObject *_ecx_kd(PyObject *self, PyObject *args){
    ...
    if(PyFloat_Check(T_obj) == 1){
        value = ecx_capi_kd(PyFloat_AsDouble(T_obj), gas, solvent, strlen(gas), strlen(solvent));
        return Py_BuildValue("d", value);
    }else if(PyLong_Check(T_obj)==1){
        value = ecx_capi_kd(PyLong_AsDouble(T_obj), gas, solvent, strlen(gas), strlen(solvent));
        return Py_BuildValue("d", value);
    }else if(PyObject_CheckBuffer(T_obj)==1){
        mview = PyMemoryView_FromObject(T_obj);
        buffer = PyMemoryView_GET_BUFFER(mview);
        ...
        if(strcmp(buffer->format, "d")!=0){
            ...
            for(i=0; i<buffer->shape[0]; i++){
                T =  *(((double * )buffer->buf)+i);
                value = ecx_capi_kd(T, gas, solvent, strlen(gas), strlen(solvent));
                *(((double *) new_buffer.buf)+i) = value;
            }
            new_mview = PyMemoryView_FromBuffer(&new_buffer);
            return new_mview;
...
static PyMethodDef myMethods[] = {
    {"kh", (PyCFunction) _ecx_kh, METH_VARARGS, ecx_kh_doc},
    {"kd", (PyCFunction) _ecx_kd, METH_VARARGS, ecx_kd_doc},
    { NULL, NULL, 0, NULL }
};
